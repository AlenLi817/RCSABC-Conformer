...
class ConvBlock(BaseModule):
    ...
    def forward(self, x, fusion_features=None, out_conv2=True):
        ...
        #RCSA module
        identity = torch.unsqueeze(identity, dim=0)
        identity = torch.reshape(identity,(-1,x.shape[1], x.shape[2], x.shape[3], x.shape[4]))

        x_l = F.cosine_similarity(x,identity,dim=1)
        x_e_s = torch.exp(x_l)
...
class FCUDown(BaseModule):
    ...
    def forward(self, x, x_t):
        ...
        #IC-FCU module
        x_f = self.sample_pooling(x).flatten(2).transpose(1, 2)
        x_r = self.sample_pooling(-x).flatten(2).transpose(1, 2)
        x = torch.min(x_f, x_r)
...
class IcfcuDown_Conformer(BaseBackbone):
    """Conformer backbone.
    ...
    def init_weights(self):
        super(IcfcuDown_Conformer, self).init_weights()
